...
const CPP_KEYWORDS = {
  ...
  constexpr: TokenTypes.CONST,
  void: TokenTypes.VOID,
  auto: TokenTypes.TYPE,
  int: TokenTypes.TYPE,
...
export function tokenizeCpp(code) {
  ...
  while (pos < len) {
    ...
    // Preprocessor directive handling (#include <iostream>, #define, etc.)
    if (ch === '#') {
    ...
    if (isDigit(ch)) {
      ...
      if (pos < len && /[uUlLfF]/.test(code[pos])) pos++;
      tokens.push({
        type: TokenTypes.NUM,
    ...
    if (isIdentStart(ch)) {
      ...
      if (CPP_KEYWORDS[word]) {
        tokens.push({
          type: CPP_KEYWORDS[word],
          original: word,
          line,
          column: startCol,
        });
